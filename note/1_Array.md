...
인터페이스에는 표준, 규칙, 규약, 약속 등을 정의하는데 쓰인다.

아주 유치한 예시지만 만약 여러 운동선수들이 있는데, 
축구선수는 공을 차고, 야구선수는 공을 배트로 날리고, 배구 선수는 손으로 공을 넘기는 등 다른 행위를 할 수 있다면
공통 분모인 사람이 가지는 특성들은 빼서 재사용 하는 것을 목적으로 한다.
...
따라서 재사용성은 챙기면서 느슨한 관계인 디커플링을 잘 활용할 수 있게 해주는 것이 인터페이스이다.
