...
public class Ex02_Oracle_DML_UPDATE {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 무식한 코드 방법 -> 파라미터가 달리 와도 파싱해버리므로 미리 컴파일시키고
            // 파라미터만 보낼 수 있는 성능이 좋은 preparedStatement를 사용 > valuse(?,?,?)
            // String sql = "update dmlemp set = sal = 0 where deptno = 10" + deptno;

            String sql = "update dmlemp set = sal = 0 where deptno = 10" + deptno;
            int resultRow = stmt.executeUpdate(sql); //문제가 터지면 여기서 이미 터져서 아래의 if문을 안타고 catch로 간다.
