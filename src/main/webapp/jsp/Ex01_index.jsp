...
<body>
    <h2>현재 날짜: <%= new java.util.Date() %></h2>
    <%-- JSP 주석: 브라우저 소스보기에서 안 보임 --%>

    <%
        // java 코드를 쓰는 영역
        // jsp 파일에서는 우리가 원하는 모든 작업 가능 (DB연결 > 실행 > 결과 > 화면 출력) > jsp (웹에서 필요한 모든 작업 가능)
        // jsp 장점: 빠르고 , 쉬워요 ... 모든 개발자 쉽게 웹을 만들게 해준다. 비슷한 것들로 ASPX,PHP가 있다.
        // jsp 단점: 분업화 힘들어요 , 유지보수 ( 모든 코드가 한곳에 있어서 최악) >> 관심사가 분리 되어 있지 않아요

        Date date = new Date();
        out.println("<p>스크립트릿에서 만든 Date 객체: " + date + "</p>");
        ...
        /*
            ...
            분업 하자
            Model (DTO , DAO)  >> 순수한 JAVA (POJO)
            View  (UI)         >> JSP
            Controller(중앙통제) >> Servlet

            MVC 패턴으로 현재까지 개발 되고 있어요 >> 추가적으로 MSA등등 나오고 있어요
            가끔 면접에서 물어보는 모델1과 모델2가 뭐냐고 묻는다면,
            >> model1 (JSP 모든 요청과 응답을 처리 ) + Model  >> 컨트롤러없이 동생에게 간단한 게시판 하나 만들어 주고 싶어요
            유지보수 확장이 어려워서 회사에서는 못쓰니까.
            >> model2 (Servlet 통한 요청과 응답을 처리 ) + MVC >> model2 기반의 MVC 패턴으로 프로젝트 해 보았어 >> 기업환경

            서버는 웹서버(아파치)와 와스(톰캣)로 나누어짐
            html 서비스
            웹서버 있으면 되요 (아파치) - 정적인 파일만 뿌려줌
            ...
            jsp or servlet 사용 (서버로 기술) - 동적인 부분을 할 수 있음
            ...
            상식 퀴즈)
            asp (MS) , aspx (MS) , php , jsp 공통점은 무엇일까
            ...
            2025년 현재는 JSP로 UI를 만들지않고 대체할 템플릿을 사용한다. MPA 멀티페이지 어플리케이션

            (타임리프-스프링진영에서 적극 지원한다. 확장자가 html이며 스크립터를 쓴다,
            머스테치-심플하고 확장자가 머스테치이다.콧수염 }}이 있지만 문법을 또 배워야함,
            프리마커 - 템플릿 엔진으로는 너무 과다한 많은 기능을 지원한다.)

            <-> SPA(싱글페이지) 리액트(react)와 뷰(vue)
            인터넷이 점점 좋아져서 클라이언트 컴퓨터가좋아져서 점점 서버에 부담을 줄이는 SPA 방식으로 넘어가는 추세이다.
        */
    %>
<%
    /* 자바 코드 사용 영역 - 스크립트릿
    ...
    JSP 구성요소
    1. 선언부 : page language = "java" > page 지시자로 사용언어, 인코딩, import를 지정
    2. 스크립트 요소
    2-1. 스트립트릿 <% 안에 java코드 사용 영영 *>
    2-2. 표현식 - 화면에 어떻게 출력할지 방법을 정하는지. 출력 대상은 클라이언트의 브라우저 이다. <%= response.Writer
...
<%
    ...
    // 이 페이지의 공통함수 ( 페이지 스코프로 페이지 떠나면 사용할 수 없다.)
